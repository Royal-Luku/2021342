...
async def pm_spoll_tester(bot, query):
    ...
    if not movies:
        return await query.answer("Button Links Expired !", show_alert=True)
    ...
    if files:
        ...
        k = await query.message.edit('<b>Tʜɪs Anime Nᴏᴛ Aᴠᴀɪʟᴀʙʟᴇ Iɴ Mʏ DᴀᴛᴀBᴀsᴇ Rɪɢʜᴛ Nᴏᴡ..! \n\nSᴏᴍᴇ Cᴏᴍᴍᴏɴ Rᴇᴀsᴏɴs ⤵️\n\n☞ Rᴇǫᴜᴇsᴛᴇᴅ Aɴɪᴍᴇ Nᴏᴛ Aᴅᴅᴇᴅ Iɴ Mʏ Dᴀᴛᴀʙᴀsᴇ\n\n☞ Aɴɪᴍᴇ Nᴏᴛ Rᴇʟᴇᴀsᴇᴅ</b>', reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Request From Owner", user_id=int(1119579816))]])) 
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if imdb:
        ...
        cap = f"I think this is your Requested Result `{search}`"
...
async def pm_spoll_choker(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("<b>Tʜɪs Anime Nᴏᴛ Aᴠᴀɪʟᴀʙʟᴇ Iɴ Mʏ DᴀᴛᴀBᴀsᴇ Rɪɢʜᴛ Nᴏᴡ..! \n\nSᴏᴍᴇ Cᴏᴍᴍᴏɴ Rᴇᴀsᴏɴs ⤵️\n\n☞ Rᴇǫᴜᴇsᴛᴇᴅ Aɴɪᴍᴇ Nᴏᴛ Aᴅᴅᴇᴅ Iɴ Mʏ Dᴀᴛᴀʙᴀsᴇ\n\n☞ Aɴɪᴍᴇ Nᴏᴛ Rᴇʟᴇᴀsᴇᴅ</b>", reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Request From Owner", user_id=int(1119579816))]]))
        await asyncio.sleep(15)
        await k.delete()
        return
    ...
    if not movielist:
        k = await msg.reply("<b>I Couldn't Find Anything Related To That. Check your Spelling Below</b>")
    ...
    await msg.reply("<b>I Couldn't Find Anything Related To That. Check your Spelling Below</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=msg.id)
